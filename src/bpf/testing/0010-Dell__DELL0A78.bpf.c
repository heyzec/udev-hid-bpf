...
/*
 * Fix for Dell DELL0A78:00 27C6:0D42 touchpad that forgets to send
 ...
 *
 * Report ID 4 (Touch) structure from HID report descriptor:
 *
 * ...
 ...
 * 0x26, 0x7f, 0x07,              //   Logical Maximum (1919)            115
 ...
 * 0x46, 0xbb, 0x01,              //   Physical Maximum (443)            128
 * 0x95, 0x01,                    //   Report Count (1)                  131
 * 0x81, 0x02,                    //   Input (Data,Var,Abs)              133
 * 0x46, 0x31, 0x01,              //   Physical Maximum (305)            135
 * 0x26, 0x37, 0x04,              //   Logical Maximum (1079)            138
 ...
 * ...
 *
 * - Device supports up to 5 contacts
 * - Each contact: 1 byte flags + 4 bytes coordinates (5 bytes total)
 * - Flags byte: Confidence(0) | Tip Switch(1) | padding(2-3) | Contact ID(4-7)
 ...
 *
 * Note: This device has a different flag layout compared to Synaptics DLL0945
 * (Contact ID is in bits 4-7 instead of bits 2-4)
 */
...
#define VID_DELL		0x27C6
#define PID_DELL0A78		0x0D42

/* Report ID for touch events */
#define TOUCH_REPORT_ID		4

/* Max number of contacts supported by this implementation */
#define MAX_CONTACTS		5

/* Expected report descriptor size for Dell DELL0A78 */
...
int BPF_PROG(dell_dell0a78_device_event, struct hid_bpf_ctx *hctx)
{
...
HID_BPF_OPS(dell_dell0a78) = {
	.hid_device_event = (void *)dell_dell0a78_device_event,
};
...
int probe(struct hid_bpf_probe_args *ctx)
{
	 ...
	 * finger release events on Dell DELL0A78 touchpads until the
...
HID_BPF_CONFIG(
	HID_DEVICE(BUS_I2C, HID_GROUP_MULTITOUCH_WIN_8, VID_DELL, PID_DELL0A78),
);

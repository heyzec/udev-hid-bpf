...
@pytest.mark.parametrize("source", ["0011-Synaptics__DLL0945"])
class TestSynapticsDLL0945:
    ...
    def test_single_contact_normal_operation(self, bpf, source):
        ...
        # Report ID 3, contact count at offset 25
        # Single contact at offset 1: active contact 0 at (100, 200)
        report = bytearray(31)
        report[0] = 3  # Report ID
        report[1] = 0x03  # Confidence (0x01) | Tip Switch (0x02) | Contact ID 0
    ...
    def test_missing_finger_release_synthesis(self, bpf, source):
        ...
        # Simulate the problematic sequence from timestamps 000085.934592 and 000085.948315

        # First report: 3 active contacts (IDs 0, 1, 2)
        report1 = bytearray(31)
        report1[0] = 3  # Report ID

        # Contact 0: active at (474, 335)
        report1[1] = 0x01 | 0x02  # Confidence | Tip Switch, Contact ID 0
        report1[2] = 474 & 0xFF
        ...
        report1[6] = 0x01 | 0x02 | (1 << 2)  # Confidence | Tip Switch | Contact ID 1
        report1[7] = 500 & 0xFF
        ...
        report1[11] = 0x01 | 0x02 | (2 << 2)  # Confidence | Tip Switch | Contact ID 2
        report1[12] = 600 & 0xFF
        ...
        report2[0] = 3  # Report ID

        # Contact 1: properly released (confidence but no tip switch, coordinates maintained)
        report2[1] = (1 << 2) | 0x01  # Contact ID 1 with confidence, no tip switch
        report2[2] = 500 & 0xFF
        ...
        for i in range(5):  # MAX_CONTACTS = 5
            ...
            if offset + 4 < len(result2_array):
                ...
                contact_id = (flags & 0x1C) >> 2  # Extract contact ID
    ...
    def test_gradual_contact_release(self, bpf, source):
        ...
        # Contact 1: active
        report1[6] = 0x07  # Confidence | Tip Switch | Contact ID 1
        ...
        report2[6] = 0x07  # Confidence | Tip Switch | Contact ID 1
